...
| L8 | Functions can take/return a struct by value as a plain struct variable or a by-value member-access sub-expression (`take(b.corner)`, `bussin b.corner;`) of the exact matching type — a struct-returning *call result* (`take(make_point())`) and a pointer-to-struct return type are not yet supported. | [ast.c:4480](../ast.c#L4480) |
