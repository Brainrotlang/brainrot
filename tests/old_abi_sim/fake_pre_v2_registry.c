 ...
 * NOT include stdrot_api.h and does NOT define stdrot_get_api_v2() --
 * only the OLD symbol, under the OLD name, with the OLD layout -- so a
 ...
 * (stdrot.c) detects this via dlsym("stdrot_get_api_v2") failing cleanly
...
/* The OLD symbol name -- deliberately NOT stdrot_get_api_v2(). */
